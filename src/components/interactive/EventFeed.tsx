...
import { events, type EventSource, type DscStatus } from '../../data/events'
  ...
  return (
    <div>
            ...
            <div key={e.id} className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between md:p-8">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  <p className="label-mono text-[0.65rem] text-cream-wash">{e.source}</p>
              ...
              <div className="shrink-0">
                <LinkButton to={e.link} variant="ghost" className="!px-5 !py-2.5 whitespace-nowrap">
                  {e.dscStatus === 'Ambassadors Attending' ? 'Onboard IRL' : 'View event'}
                </LinkButton>
              </div>
            </div>
          ))
      ...
      <p className="label-mono mt-6 text-[0.62rem] text-cream-wash/70">
        ...
        , or{' '}
        <a href="https://sheeets.xyz" target="_blank" rel="noreferrer" className="underline decoration-cream-wash/40 underline-offset-2 hover:text-cream">
          sheeets.xyz
        </a>
        .
      </p>
